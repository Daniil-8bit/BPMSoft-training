namespace BPMSoft.Configuration
{
	...
	using BPMSoft.ExchangeApi.Interfaces;
	...
	public class ExchangeUtilityImpl: IExchangeUtility
	{
		...
		private string GetToken(UserConnection userConnection, string senderEmailAddress) {
			...
			OAuthClient oauthClient = (OAuthClient)Activator.CreateInstance(Type.GetType("BPMSoft.Configuration." + oauthClassName),
		...
		private void CreateNewAttach(UserConnection userConnection, Exchange.Attachment attachment,
			...
			if (emailAttachment != null) {
				...
				try {
					...
					RaiseAttachmentError(userConnection, baseException.Message, fileId);
				}
			}
		}
		...
		/// <summary>
		/// Handles Item bind error.
		/// </summary>
		/// <param name="error"><see cref="Exception"/> instance.</param>
		/// <param name="id"><see cref="Exchange.ItemId"/> instance.</param>
		private void ProcessingItemBindException(Exception error, Exchange.ItemId id) {
			...
			if (IsItemProcessedErrorMessage(message)) {
				Log.ErrorFormat("[ExchangeUtility.SafeBindItem]: Error while loading item with Id: {0}", baseException, id);
			...
			try {
				...
				if (credentials.IsAutodiscover) {
					exchangeService.AutodiscoverUrl(senderEmailAddress, delegate (string url) {
						var regexString = BPMSoft.Core.Configuration.SysSettings.GetValue<string>(userConnection,
		...
		public T SafeBindItem<T>(Exchange.ExchangeService service, Exchange.ItemId id)
			...
			try {
				...
				ProcessingItemBindException(exception, id);
			}
			return value;
		}
		...
		/// and returns a hierarchical collection <see cref="BPMSoft.Configuration.ExchangeMailFolder"/>.
		/// </summary>
		...
		/// <returns>Collection objects of <see cref="BPMSoft.Configuration.ExchangeMailFolder"/>.
		/// </returns>
		...
		public Exchange.ExchangeService CreateExchangeService(UserConnection userConnection,
			...
			Log.InfoFormat(logTemplate, exchangeCredentials.ServerAddress,
					((BPMSoft.Mail.Credentials)(exchangeCredentials)).UserName, senderEmailAddress);
		...
		public void RemoveAllSyncJob(UserConnection userConnection, string senderEmailAddress, Guid serverTypeId) {
			if (serverTypeId != Guid.Empty && serverTypeId == ExchangeConsts.ExchangeMailServerTypeId) {
				RemoveSyncJob(userConnection, senderEmailAddress, ExchangeConsts.MailSyncProcessName);
				RemoveSyncJob(userConnection, senderEmailAddress, ExchangeConsts.ContactSyncProcessName);
				RemoveSyncJob(userConnection, senderEmailAddress, ExchangeConsts.ActivitySyncProcessName);
			}
		}
		...
		public void UploadAttachmentsData(UserConnection userConnection, string userEmailAddress) {
			...
			} catch (Exception e) when (e.GetBaseException() is Exchange.ServiceResponseException) {
				return;
			}
			SetSecurityProtocolOptions(userConnection);
