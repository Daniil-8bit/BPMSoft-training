namespace BPMSoft.Configuration
{
	...
	public class SysSyncMetaDataActualizer : ISynchronizationController
	{
		...
		private void ActualizeDetailMetaData(UserConnection userConnection, MetaDataInfo metaDataInfo) {
			...
			var disabledActivityParticipantCondition = userConnection.GetIsFeatureEnabled("NewMeetingIntegration")
				&& metaDataInfo.EntitySchemaName == "ActivityParticipant";
			if (metaDataInfo.ParentRemoteId.IsNullOrEmpty() || disabledActivityParticipantCondition) {
				return;
			}
