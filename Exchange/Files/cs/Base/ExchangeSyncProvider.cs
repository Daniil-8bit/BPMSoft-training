namespace BPMSoft.Sync.Exchange
{
	...
	public abstract class ExchangeSyncProvider : BaseExchangeSyncProvider
	{
		...
		/// <summary>
		/// Current time zone of BPMCRM user.
		/// </summary>
		/// <remarks>
		/// It converts a BPMCRM local time server
		/// to the user's local time when using the local storage and 
		/// converts local user time to a local time server BPMCRM, 
		...
		protected virtual List<Exchange.Folder> SafeBindFolders(Exchange.ExchangeService service,
			...
			foreach (string uniqueId in folderRemoteIds) {
				...
				try {
					...
					result.Add(folder);
				} catch (Exception exception) when (exception.GetBaseException() is Exchange.ServiceResponseException) {
