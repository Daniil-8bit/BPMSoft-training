namespace BPMSoft.Sync.Exchange
{
	...
	using ItemNotFoundException = BPMSoft.Common.ItemNotFoundException;
	using SysSyncMetaData = BPMSoft.Core.Configuration.SysSyncMetaData;
	...
	public class ExchangeAppointmentSyncProvider : ExchangeActivitySyncProvider
	{
		...
		/// <summary>
		/// <see cref="BPMSoft.Configuration.SynchronizationErrorHelper"/> instance.
		/// </summary>
		...
		public override void OnLocalItemAppliedInRemoteStore(SyncContext context, IRemoteItem remoteItem,
			...
			if (remoteItem.Action == SyncAction.Delete && remoteItem.State == SyncState.Deleted) {
				EntitySchema schema = context.UserConnection.EntitySchemaManager.GetInstanceByName("ActivityParticipant");
				...
				foreach (Guid activityId in entityIds) {
					Entity activityParticipant = schema.CreateEntity(context.UserConnection);
					activityParticipant.UseAdminRights = false;
					if (activityParticipant.FetchFromDB(new Dictionary<string, object> {
						{"Activity", activityId },
						{"Participant", context.UserConnection.CurrentUser.ContactId }
					}, false)) {
						activityParticipant.SetColumnValue("InviteResponseId", ActivityConsts.ParticipantResponseDeclinedId);
						localItem.AddOrReplace("ActivityParticipant", new SyncEntity(activityParticipant, SyncState.Modified) {
