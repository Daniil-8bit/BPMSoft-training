namespace BPMSoft.Sync.Exchange
{
	using System;
	using System.Data;
	...
	public class ExchangeAppointment : ExchangeBase
	{
		...
		private SyncAction GetParticipantSyncAction(Entity participant, Exchange.Attendee attendee) {
			...
			if (participant.StoringState == BPMSoft.Core.Configuration.StoringObjectState.New) {
		...
		private void ActualizeSyncParticipantsList(List<Guid> modifiedParticipants, LocalItem localItem) {
			...
			foreach (SyncEntity entity in entities.Where(e => !modifiedParticipants.Any(p => e.EntityId == p))) {
				...
				if (participant.StoringState == BPMSoft.Core.Configuration.StoringObjectState.Deleted) {
		...
		private void SyncAllAttendees(SyncContext context, Entity activity, Exchange.Appointment exchangeAppointment,
			...
			var organizerEmail = GetAppointmentOrganizerEmail(exchangeAppointment);
		...
		private bool CheckNotDeleted(SyncContext context, LocalItem localItem) {
			if (IsDeletedProcessed("Activity", ref localItem) ||
				...
				LogInfo(context, "Appointment {0} deleted in bpmcrm, fill local item skipped.", GetDisplayName());
				return false;
			}
			return true;
		}
		...
		/// <summary>
		/// Locks item for synchronization to bpmcrm.
		/// </summary>
		/// <param name="context"><see cref="SyncContext"/> instance.</param>
		...
		private bool LockItem(SyncContext context) {
			if (GetRemoteItemLockedForSync(context)) {
				LogInfo(context, "Appointment {0} locked for sync in bpmcrm (Id = {1}), sync action skipped.", GetDisplayName(), Id);
		...
		/// <param name="context"><see cref="SyncContext"/> instance.</param>
		/// <param name="localItem"><see cref="LocalItem"/> instance.</param>
		/// <returns><c>True</c> when current user can be appointment organizer in bpmcrm.
		/// Otherwise returns <c>false</c>.</returns>
		private bool CheckOrganizer(SyncContext context, LocalItem localItem) {
			...
			if (!GetCanSaveChangesToBpm(context.UserConnection, organizerEmail)) {
				LogInfo(context, "Appointment {0} has organizer in bpmcrm (organizer = {1}), sync action skipped.", GetDisplayName(),
					organizerEmail);
				SetLocalItemSchemasAction(context, localItem, SyncAction.None);
				Action = SyncAction.None;
				if (context.UserConnection.GetIsFeatureEnabled("SyncDeletedAppointments") && exchangeAppointment.IsCancelled) {
					var activitySyncEntity = localItem.Entities["Activity"][0];
					var activity = activitySyncEntity.Entity;
					string exchangeSubject = ActivityUtils.FixActivityTitle(exchangeAppointment.Subject, context.UserConnection,
		...
		/// <param name="context"><see cref="SyncContext"/> instance.</param>
		/// <param name="localItem"><see cref="LocalItem"/> instance.</param>
		/// <returns><c>True</c> when appointment unique and can be created in bpmcrm.
		/// Otherwise returns <c>false</c>.</returns>
		private bool CheckAppointmentContentDuplicates(SyncContext context, LocalItem localItem) {
			...
			Entity activity = context.UserConnection.GetIsFeatureEnabled("ExchangeRecurringAppointments")
				? GetActivityIgnoreRights(context, localItem)
		...
		private Entity GetActivityIgnoreRights(SyncContext context, LocalItem localItem) {
			var select = new Select(context.UserConnection).Top(1).Column("LocalId")
				.From("SysSyncMetaData").Where("SyncSchemaName").IsEqual(Column.Const("Activity"))
				.And("RemoteId").IsEqual(Column.Parameter(Id)) as Select;
			var activityId = select.ExecuteScalar<Guid>();
			var schema = context.UserConnection.EntitySchemaManager.GetInstanceByName("Activity");
			var activity = schema.CreateEntity(context.UserConnection);
			activity.UseAdminRights = false;
			if (localItem.Entities["Activity"].Count == 0 && activityId != Guid.Empty && activity.FetchFromDB(activityId)) {
				var syncEntity = SyncEntity.CreateNew(activity);
				syncEntity.Action = SyncAction.Update;
				localItem.AddOrReplace("Activity", syncEntity);
				return activity;
			}
			return GetEntityInstance<Entity>(context, localItem, "Activity");
		}
		...
		/// </summary>
		/// <returns>True if current system is Unix.</returns>
		...
		/// <summary>
		/// Checks may current user save <see cref="ExchangeBase.Item"/> changes to bpmcrm.
		/// Current user may save external item changes to bpmcrm if he is appointment organizer, 
		/// or when appointment organizer appointment synchrodization not exists.
		/// </summary>
		/// <param name="userConnection"><see cref="UserConnection"/> instance.</param>
		/// <param name="organizerEmail">Appointment organizer email.</param>
		/// <returns><c>True</c> if current user can save <see cref="ExchangeBase.Item"/> changes to bpmcrm, 
		/// <c>false</c> otherwise.</returns>
		...
		protected void ClearAttendees(UserConnection userConnection, Exchange.Appointment exchangeAppointment) {
			...
			string organizerEmail = GetAppointmentOrganizerEmail(exchangeAppointment);
			List<Guid> contactIds = ContactUtilities.FindContactsByEmail(organizerEmail, userConnection);
		...
		/// <summary>
		/// Validates exchange appointment before synchronization to bpmcrm.
		/// </summary>
		/// <param name="context"><see cref="SyncContext"/> instance.</param>
		/// <param name="localItem"><see cref="LocalItem"/> instance.</param>
		/// <returns><c>True</c> when appointment can be synchronized to bpmcrm.
		/// Otherwise returns <c>false</c>.</returns>
		...
		public void SetExchangeAppointmentProporties(SyncContext context, Entity activity,
			...
			exchangeAppointment.StartTimeZone = GetExchangeAppointmentTimeZoneInfo(userConnection, startDate);
			var endDate = activity.GetTypedColumnValue<DateTime>("DueDate");
			exchangeAppointment.End = endDate;
			exchangeAppointment.EndTimeZone = GetExchangeAppointmentTimeZoneInfo(userConnection, endDate);
