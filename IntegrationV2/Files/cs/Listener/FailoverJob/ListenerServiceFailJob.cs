namespace BPMSoft.Configuration
{
	...
	public class ListenerServiceFailJob: IJobExecutor
	{
		...
		private List<Mailbox> GetMailboxesWithoutSubscriptions( ) {
			...
			} else if (GetIsFeatureDisabledForAnyUser(UserConnection, "DoNotSkipFailover")) {
				_log.Warn($"Listener unavailable, skips failover for {mailboxes.Count} mailboxes");
				return new List<Mailbox>();
			}
			if (!NeedProceed(mailboxes)) {
				_log.DebugFormat("GetMailboxesWithoutSubscriptions method ended");
		...
		/// <summary>
		/// Checks is feature disabled for any user.
		/// </summary>
		/// <param name="userConnection"><see cref="UserConnection"/> instance.</param>
		/// <param name="code">Feature code.</param>
		/// <returns><c>True</c> if feature disabled for any user. Returns <c>false</c> otherwise.</returns>
		private bool GetIsFeatureDisabledForAnyUser(UserConnection userConnection, string code) {
			return !GetIsFeatureEnabledForAnyUser(userConnection, code);
		}
