namespace BPMSoft.IntegrationV2.Utils
{
	using System;
	...
	using TS = BPMSoft.Web.Http.Abstractions;
	...
	internal class ListenerUtils
	{
		...
		/// <summary>
		/// Create new value of BpmCrmExchangeEventsEndpointUrl system setting.
		/// </summary>
		/// <returns>New value of BpmCrmExchangeEventsEndpointUrl system setting.</returns>
		private string GetNewBpmCrmExchangeEventsEndpointUrl() {
		...
		public static void TryDoListenerAction(Action action, string senderEmailAddress, UserConnection uc) {
			try {
				...
				if (GetIsFeatureDisabled(uc, "OldEmailIntegration") && GetIsFeatureEnabled(uc, "HandleIntNetworkError")) {
		...
		/// <summary>
		/// Returns bpmcrm new email events endpoint uri.
		/// </summary>
		/// <returns>bpmcrm new email events endpoint uri.</returns>
		public string GetBpmEndpointUrl() {
			var endpointUrl = SysSettings.GetValue(_uc, "BpmCrmExchangeEventsEndpointUrl", "");
			if (endpointUrl.IsEmpty()) {
				endpointUrl = GetNewBpmCrmExchangeEventsEndpointUrl();
				SysSettings.SetDefValue(_uc, "BpmCrmExchangeEventsEndpointUrl", endpointUrl);
			}
			if (endpointUrl.IsEmpty()) {
				throw new InvalidObjectStateException("BpmCrm exchange events endpoint url cannot be created. " +
					"Fill BpmCrmExchangeEventsEndpointUrl system setting.");
			}
			return endpointUrl;
		}
